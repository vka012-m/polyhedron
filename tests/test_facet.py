...
class TestVoid(unittest.TestCase):
    ...
    def test_vertical01(self):
        edges = []
        f = Facet([R3(0.0, 0.0, 0.0), R3(3.0, 0.0, 0.0),
                   R3(0.0, 3.0, 0.0)], edges)
    ...
    def test_h_normal01(self):
        ...
        f = Facet([R3(0.0, 0.0, 0.0), R3(3.0, 0.0, 0.0),
                   R3(0.0, 3.0, 0.0)], edges)
        self.assertEqual(R3CollinearMatcher(f.h_normal()),
                         R3(0.0, 0.0, 1.0))

    # Нормаль к этой грани тоже направлена вертикально вверх
    def test_h_normal02(self):
        edges = []
        f = Facet([R3(0.0, 0.0, 0.0), R3(0.0, 3.0, 0.0),
                   R3(3.0, 0.0, 0.0)], edges)
        self.assertEqual(R3CollinearMatcher(f.h_normal()),
                         R3(0.0, 0.0, 1.0))
    ...
    def test_h_normal03(self):
        edges = []
        f = Facet([R3(1.0, 0.0, 0.0), R3(0.0, 1.0, 0.0),
                   R3(0.0, 0.0, 1.0)], edges)
        self.assertEqual(R3CollinearMatcher(f.h_normal()),
                         R3(1.0, 1.0, 1.0))
    ...
    def test_v_normal03(self):
        ...
        f = Facet([R3(1.0, 0.0, 0.0), R3(0.0, 1.0, 0.0),
                   R3(0.0, 0.0, 1.0)], edges)
        normals = [R3(0.0, -1.0, 0.0), R3(1.0, 1.0, 0.0),
                   R3(-1.0, 0.0, 0.0)]
        for t in zip(f.v_normals(), normals):
            self.assertEqual(R3CollinearMatcher(t[0]), t[1])
    ...
    def test_center01(self):
        ...
        f = Facet([R3(0.0, 0.0, 0.0), R3(2.0, 0.0, 0.0),
                   R3(2.0, 2.0, 0.0), R3(0.0, 2.0, 0.0)], edges)
        self.assertEqual(R3ApproxMatcher(f.center()),
                         (R3(1.0, 1.0, 0.0)))
